...
\begin{Teo}\label{ttc}
Un mapa $f\colon \abs{K}\rightarrow X$ es continuo si y sólo si $f|_{\sigma}$ es continua para cada $\sigma$ en $\abs{K}$
\end{Teo}
\begin{Teo}
$\abs{K}$ es Hausdorff
\end{Teo}
...
Primero notemos que $\abs{K}$ es la unión finita de cerrados en $\mathbb{R}^N$ de donde se sigue que $\abs{K}$ es cerrado en $\mathbb{R}^N$. Por lo tanto los cerrados en $\abs{K}$ con la topología usual son cerados en $\mathbb{R}^N$ que están contenidos en $\abs{K}$.
