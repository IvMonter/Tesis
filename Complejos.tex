...
\begin{Prop}
Si $\textit{L}$ es una subcolleción de un complejo simplcial $\textit{K}$, tal que contiene todas las caras de sus elementos, entonces $\textit{L}$ es un subcomplejo de $\textit{K}$.
\end{Prop}
...
\end{Teo}
El espacio $\abs{K}$ es llamado el espacio subyacente de $K$ o el politopo de $K$
...
Para todo  $x= \sum_{i=0}^{n}t_i(x)v_i$ en $\sigma$ se tiene que $\abs{f}(x)= \sum_{i=0}^{n}t_i(x)f(v_i)$.
\end{Teo}
