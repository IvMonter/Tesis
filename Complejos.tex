...
Supongamos que $\tau$ es un $(n+1)-$simplejo, entonces para cada cara propia de $\sigma$ de $\tau$, sabemos que $g(\abs{\sigma})\subset C(\sigma)\subset C(\tau)$, por lo tanto $g(\abs{Bd \tau})$ está contenido en $C(\tau)$ y como $C(\tau)$ es contraíble por el teorema \ref{ext_general} $g$ puede ser extendida continuamente a $\abs{\tau}$ de modo que $g(\abs{\tau}) \subset C(\tau)$. 
Luego como $g$ es continua en cada simplejo se sigue que $g$ es continua en $\abs{K}$.
...
Definamos $H\colon \abs{K}\times\{0,1\} \to X$ por $H(x,0) = f(x)$ y $H(x,1) = f(x)$.
...
Supongamos que $\tau$ es un $(n+1)-$simplejo, entonces para cada cara propia de $\sigma$ de $\tau$, sabemos que $\abs{\sigma}\times$ está contenida en $C(\sigma)$ y que $C(\sigma)$ está contenido en $C(\tau)$, esto para toda $\sigma$ cara propia de $\tau$. Por lo tanto $H(\abs{Bd\tau})$ está contenida en $C(\tau)$. Por lo tanto $H$ puede ser extendida continuamente a $\abs{\tau}\times I$ de modo que $H(\abs{\tau}\times I)$ está contenida en $C(\tau)$.
